...
import { buscarCursosMatriculados } from '../backend/mostrarCursosMatriculados'
import { PersonajeManga } from './CursoPersonajeManga'
...
export function InicioLogin(){
    ...
    useEffect(() => {
        buscarCursosMatriculados(localStorage.getItem('tokenCorreo'))
            .then(data => {
                console.log(data)
                setCursos(data)
                data.map((item) => {
                    switch(item.id_categoria){
                        case 1 : setMostrarCategoria1(true);break;
                        case 2 : setMostrarCategoria2(true); break;
                        case 3 : setMostrarCategoria3(true); break;
                    }

                    switch(item.id_curso){
                        case 1: setMostrarCursoPersonaje(true); break;
                        case 2: setMostrarCursoIlustrator(true); break;
                        case 3: setMostrarCursoCorel(true); break;
                    }
                })
            })
    },[])
    ...
    const ApartadoCategoria1 = () => {
        return(
            <div className='Categoria-container'> 
                <div className='Categoria-titulo'><p>Ilustración y Dibujos</p></div>
                <div className='Cursos'>
                    {mostrarCursoPersonaje ? <TipoCurso imagen={personajeManga} nombre="Personaje Manga" path="/#"/> : ""}
                </div>
            </div>
        )
    }
    ...
    const ApartadoCategoria2 = () => {
        return(
            <div className='Categoria-container'>
                ...
                <div className='Cursos'>
                    {mostrarCursoIlustrator ? <TipoCurso imagen={Ilustrator} nombre="Ilustrator" />: ""}
                    {mostrarCursoCorel ? <TipoCurso imagen={CorelDraw} nombre="Corel Draw" /> : ""}
                </div>
            </div>
        )
    }
